            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel17">All Products Form</h4>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-12 col-12">
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="descriptionID" class="form-control" placeholder="Description ID" id="descriptionID" required></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-12 col-12">
                                                        <div class="form-group">
                                                            <label for="descriptionENG">Description ENG</label>
                                                            <textarea name="descriptionENG" class="form-control" placeholder="Description ID" id="descriptionENG" required></textarea>
                                                        </div>
                                                    </div>
...
@push('js')
    <script>
        CKEDITOR.replace('descriptionID', options);
        CKEDITOR.replace('descriptionENG', options);
        ...
        function deleted(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "get",
                        url: "{{ '/webmin/all-product/delete/' }}" + id,
                        data: {
                            id: id
                        },
        ...
        function multideleted() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "get",
                        url: "{{ route('all-product.multi_delete') }}",
                        data: {
                            id: id
                        },
                        success: function(result) {
        ...
        function updateStatus(id) {
            var sts;
            var url = "{{ '/webmin/all-product/status/' }}" + id;
        ...
        function addData() {
            ...
            var descriptionID = CKEDITOR.instances['descriptionID'].getData()
            var descriptionENG = CKEDITOR.instances['descriptionENG'].getData()
            console.log(sort, statusForm, title, thumbnail, category, descriptionID, descriptionENG)


            if (sort == '' || statusForm == '' || title == '' || thumbnail == '' || category == '' || descriptionID == '' ||
                descriptionENG == '') {
                Swal.fire({
                    title: 'Error!',
                ...
                var url = "{{ route('all-product.create') }}";
                $.ajax({
                    type: 'POST',
                    url: url,
                    data: {
        ...
        //edit
        function edit(id) {
            var url = "{{ '/webmin/all-product/edit/' }}" + id;
            $.ajax({
                type: 'GET',
                url: url,
                success: function(data) {
                    ...
                    $('#statusForm option[value="' + data.data[0].sts + '"]').attr("selected", "selected");
                    $('#categoryForm option[value="' + data.data[0].category + '"]').attr("selected",
                        "selected");
                    $('#category').val(data.data[0].category);
                    $('#title').val(data.data[0].title);
                    $('#thumbnail').val(data.data[0].image);
                    CKEDITOR.instances['descriptionID'].setData(data.data[0].desc_id)
                    CKEDITOR.instances['descriptionENG'].setData(data.data[0].desc_en)
